    ...
    <div v-if="loadedTheme" class="container mt-3">
        ...
        <div v-if="enableEditMode" class="sidebar" data-testid="edit-sidebar">
            <div class="sidebar-body">
                ...
                <!-- Access / Visibility -->
                <div class="my-3">
                    <label for="visibility" class="form-label">{{ $t("Access") }}</label>
                    <select id="visibility" v-model="config.visibility" class="form-select">
                        <option value="public">{{ $t("Public") }}</option>
                        <option value="password">{{ $t("Password protected") }}</option>
                        <option value="private">{{ $t("Login required (users)") }}</option>
                    </select>
                </div>
                <div v-if="config.visibility === 'password'" class="my-3">
                    <label for="statusPagePassword" class="form-label">{{ $t("Password") }}</label>
                    <input
                        id="statusPagePassword"
        ...
        <div :class="{ edit: enableEditMode }" class="main">
            ...
            <footer class="mt-5 mb-4">
                ...
                <p v-if="config.showPoweredBy" data-testid="powered-by">
                    {{ $t("Powered by") }}
                    <a target="_blank" rel="noopener noreferrer" href="https://github.com/drno88/uptime-kuma-fork">
                        Uptime Kuma
                    </a>
                </p>
...
export default {
    ...
    async mounted() {
        ...
        this.getData()
            .then((res) => {
                if (!res) { this.loading = false; return; } // auth required — modal shown by getData()
                this.config = res.data.config;

                // Check visibility even when data came from preloadData (SSR)
                const visibility = this.config.visibility || "public";
                ...
                if (visibility !== "public" && !spToken) {
                    if (visibility === "private") {
                        ...
                        if (userToken && userToken !== "autoLogin") {
                            ...
                            // Re-fetch data with token now set
                            this.getData().then((r) => {
                                if (!r) {
                                    // Token invalid/expired — show login form
                                    sessionStorage.removeItem("statusPageToken_" + this.slug);
                                    this.pageVisibility = visibility;
                                    this.authRequired = true;
                                    this.loading = false;
                                    return;
                                }
                                this._applyPageData(r);
                            });
                            return;
                        }
                    }
    ...
    methods: {
        ...
        _applyPageData(res) {
            this.config = res.data.config;
            if (!this.config.domainNameList) this.config.domainNameList = [];
            if (this.config.icon) this.imgDataUrl = this.config.icon;
        ...
        async submitAuth() {
            ...
            try {
                let res;
                if (this.pageVisibility === "password") {
                    res = await axios.post(`/api/status-page/${this.slug}/auth`, { password: this.authPassword });
                } else {
                    res = await axios.post(`/api/status-page/${this.slug}/login`, {
                        username: this.authUsername,
                        password: this.authPassword,
                    });
                }
                sessionStorage.setItem("statusPageToken_" + this.slug, res.data.token);
                this.authRequired = false;
                this.authPassword = "";
                this.authUsername = "";
                this.getData().then((r) => {
                    if (r) this._applyPageData(r);
                });
            } catch (e) {
                this.authError = e.response?.data?.msg || "Authentication failed";
            }
        },

        getData: function () {
            if (window.preloadData) {
                const data = window.preloadData;
                window.preloadData = null; // clear so re-calls hit API
                return Promise.resolve({ data });
            }
            const token = sessionStorage.getItem("statusPageToken_" + this.slug);
            const cfg = token ? { headers: { Authorization: "Bearer " + token } } : {};
            return axios.get("/api/status-page/" + this.slug, cfg).catch((error) => {
                if (error.response?.status === 401) {
                    this.pageVisibility = error.response.data.visibility || "password";
                    this.authRequired = true;
                    return null;
                }
                throw error;
        ...
        updateHeartbeatList() {
            ...
            if (!this.editMode) {
                ...
                axios.get("/api/status-page/heartbeat/" + this.slug, axiosConfig).then((res) => {
