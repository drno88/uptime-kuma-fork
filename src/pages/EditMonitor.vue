                ...
                <div class="shadow-box shadow-box-with-fixed-bottom-bar">
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <!-- Sharing (admin only) -->
                            <div v-if="$root.userRole === 'admin'" class="mb-4">
                                <h2 class="mt-5 mb-2">{{ $t("Share With") }}</h2>
                                <p class="form-text mb-2">{{ $t("Select users who can view this monitor") }}</p>
                                <div v-if="allUsers.length <= 1" class="text-muted small">
                                    {{ $t("No other users to share with") }}
                                </div>
                                <div
                                    v-for="user in allUsers.filter(u => u.username !== $root.username)"
                                    :key="user.id"
                                ...
                                >
                                    <input
                                        ...
                                        type="checkbox"
                                    />
                                    <label class="form-check-label" :for="'share-user-' + user.id">
                                        {{ user.username }}
                                        <span class="badge ms-1" :class="user.role === 'admin' ? 'bg-primary' : 'bg-secondary'">{{ user.role }}</span>
                                    </label>
                                </div>
                            </div>
                ...
                `
{
    ...
    methods: {
        ...
        init() {
            ...
            // Load user list and sharing info for admin
            if (this.$root.userRole === "admin") {
                ...
                if (monitorID && !this.isAdd) {
                    this.$root.getSocket().emit("getMonitorAccess", monitorID, (res) => {
                        if (res.ok) {
                            this.sharedWithUsers = res.userIDs;
                        }
                    });
                } else {
        ...
        saveMonitorAccess(monitorID) {
            if (this.$root.userRole !== "admin") {
                return Promise.resolve();
            }
            return new Promise((resolve) => {
                this.$root.getSocket().emit("setMonitorAccess", monitorID, this.sharedWithUsers, (res) => {
                    resolve(res);
                });
            });
        },
            ...
            if (this.isAdd || this.isClone) {
                this.$root.add(this.monitor, async (res) => {
                    if (res.ok) {
                        ...
                        await this.saveMonitorAccess(res.monitorID);
                ...
                await this.saveMonitorAccess(this.monitor.id);
