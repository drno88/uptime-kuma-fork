...
module.exports.statusPageSocketHandler = (socket) => {
    ...
    socket.on("saveStatusPage", async (slug, config, imgDataUrl, publicGroupList, callback) => {
        try {
            ...
            const validVisibilities = ["public", "password", "private"];
            statusPage.visibility = validVisibilities.includes(config.visibility) ? config.visibility : "public";

            if (statusPage.visibility === "password" && config.password) {
                statusPage.password = await passwordHash.generate(config.password);
            } else if (statusPage.visibility !== "password") {
