    ...
    io.on("connection", async (socket) => {
        ...
        socket.on("getUsers", async (callback) => {
            try {
                ...
                let users = await R.getAll("SELECT id, username, role, active FROM `user` ORDER BY id");
        ...
        socket.on("addUser", async (userData, callback) => {
            try {
                ...
                let existing = await R.findOne("user", " TRIM(username) = ? ", [userData.username.trim()]);
                if (existing) {
                ...
                user.role = userData.role === "admin" ? "admin" : "user";
                user.active = true;
                await R.store(user);

                callback({ ok: true, msg: "User added successfully" });
            } catch (e) {
        ...
        socket.on("deleteUser", async (userID, callback) => {
            try {
                ...
                // Remove monitor access entries
                await R.exec("DELETE FROM monitor_access WHERE user_id = ?", [userID]);

                // Transfer monitors to deleting admin (or just nullify - keep monitors)
                await R.exec("UPDATE monitor SET user_id = ? WHERE user_id = ?", [socket.userID, userID]);
        ...
        socket.on("getMonitorAccess", async (monitorID, callback) => {
            try {
                ...
                let access = await R.getAll("SELECT user_id FROM monitor_access WHERE monitor_id = ?", [monitorID]);
                callback({ ok: true, userIDs: access.map(r => r.user_id) });
            } catch (e) {
                callback({ ok: false, msg: e.message });
            }
        });

        socket.on("setMonitorAccess", async (monitorID, userIDs, callback) => {
            try {
                await checkAdmin(socket);

                let monitor = await R.findOne("monitor", " id = ? AND user_id = ? ", [monitorID, socket.userID]);
                if (!monitor) {
                    throw new Error("Monitor not found or you are not the owner");
                }

                // Helper: upsert access for a single monitor
                const grantAccess = async (mid, uids) => {
                    await R.exec("DELETE FROM monitor_access WHERE monitor_id = ?", [mid]);
                    for (let uid of uids) {
                        if (uid !== socket.userID) {
                            await R.exec(
                                "INSERT INTO monitor_access (monitor_id, user_id) VALUES (?, ?)",
                                [mid, uid]
                            );
                        }
                    }
                ...
                // Set access on the monitor itself
                await grantAccess(monitorID, userIDs);

                // If monitor is inside a group — automatically share the parent group too
                // so it appears in the tree for shared users. Walk up the chain.
                let current = monitor;
                while (current.parent) {
                    let parent = await R.findOne("monitor", " id = ? ", [current.parent]);
                    if (!parent) {
                        break;
                    }
                    if (userIDs.length > 0) {
                        // Merge: add new users to any existing access on the parent
                        let existing = await R.getAll(
                            "SELECT user_id FROM monitor_access WHERE monitor_id = ?",
                            [parent.id]
                        );
                        let existingIDs = existing.map((r) => r.user_id);
                        let merged = [...new Set([...existingIDs, ...userIDs.filter((uid) => uid !== socket.userID)])];
                        await grantAccess(parent.id, merged);
                    }
                    current = parent;
                }
...
async function afterLogin(socket, user) {
    ...
    socket.join(user.id);
    socket.emit("userRole", user.role || "user");
