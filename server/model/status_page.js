...
class StatusPage extends BeanModel {
    ...
    static async handleStatusPageResponse(response, indexHTML, slug) {
        ...
        if (statusPage) {
            // For non-public pages don't embed preloadData — let frontend handle auth via API
            const includeData = (statusPage.visibility || "public") === "public";
            response.send(await StatusPage.renderHTML(indexHTML, statusPage, includeData));
        } else {
            response.status(404).send(UptimeKumaServer.getInstance().indexHTML);
        }
    }
     ...
     */
    static async renderHTML(indexHTML, statusPage, includeData = true) {
        ...
        // Preload data — skip for non-public pages, frontend will fetch via API after auth
        if (includeData) {
