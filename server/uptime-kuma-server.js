...
class UptimeKumaServer {
    ...
    async getMonitorJSONList(userID, monitorID = null) {
        // Fetch own monitors + monitors shared with this user via monitor_access
        let sharedIDs = await R.getAll(
            "SELECT monitor_id FROM monitor_access WHERE user_id = ?",
            [userID]
        );
        let sharedIDSet = new Set(sharedIDs.map((r) => r.monitor_id));

        let query = " (user_id = ? OR id IN (SELECT monitor_id FROM monitor_access WHERE user_id = ?)) ";
        ...
        // Fetch access lists for own monitors (for sharing UI)
        const accessMap = {};
        const ownMonitorIDs = monitorList.filter((m) => m.user_id === userID).map((m) => m.id);
        if (ownMonitorIDs.length > 0) {
            const accessRows = await R.getAll(
                `SELECT monitor_id, user_id FROM monitor_access WHERE monitor_id IN (${ownMonitorIDs.map(() => "?").join(",")})`,
                ownMonitorIDs
            );
            for (const row of accessRows) {
                if (!accessMap[row.monitor_id]) {
                    accessMap[row.monitor_id] = [];
                }
                accessMap[row.monitor_id].push(row.user_id);
            }
        }
        ...
        monitorList.forEach((monitor) => {
            ...
            json.isOwner = monitor.user_id === userID;
            json.isShared = sharedIDSet.has(monitor.id);
            json.sharedWith = accessMap[monitor.id] || [];
